...
use veo_complexity::{self, AnalyzeOpts, Profile};
use veo_ffmpeg::{self, Codec, EncodeJob, Resolution};
...
pub async fn adapt(source: &str, cfg: Config) -> anyhow::Result<Result> {
    ...
    let profile = veo_complexity::analyze(source, AnalyzeOpts {
        segment_duration: cfg.segment_duration,
        subsample: 2,
    }).await?;

    // Step 2: Map complexity to initial CRF
    let mut segments: Vec<SegmentResult> = profile.segments.iter().map(|seg| {
        SegmentResult {
            start: seg.start,
            end: seg.end,
    ...
    let tmp_dir = tempfile::Builder::new().prefix("veo-persegment-").tempdir()?;
    ...
    for (i, seg) in segments.iter_mut().enumerate() {
        ...
        for iter in 0..cfg.max_iterations {
            ...
            let enc_result = veo_ffmpeg::encode(job, None).await?;
            seg.bitrate = enc_result.bitrate;

            let q_result = veo_quality::measure(
                &seg_source.to_string_lossy(),
                &seg_encoded.to_string_lossy(),
                MeasureOpts {
                    metrics: vec![Metric::Vmaf],
                    subsample: 5,
                    ..Default::default()
                },
            ).await?;
