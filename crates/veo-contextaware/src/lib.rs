...
use veo_encoding::{Config as EncodingConfig, ProgressSender};
use veo_hull::{Hull, Point};
use veo_ladder::{self, Ladder};
...
pub async fn analyze(
    ...
    for (i, profile) in cfg.profiles.iter().enumerate() {
        let pt_cfg = veo_pertitle::Config {
        ...
        let pt_result = veo_pertitle::analyze(source, pt_cfg, None).await
            .map_err(|e| anyhow::anyhow!("analysis for {} failed: {e}", profile.name))?;

        let device_ladder = veo_ladder::select(&pt_result.hull, &profile.ladder_opts);
