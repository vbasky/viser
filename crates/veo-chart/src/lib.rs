...
pub fn rd_curve(points: &[Point], hull: &Hull, opts: Opts) -> anyhow::Result<Vec<u8>> {
...

    {
        ...
        if !crossovers.is_empty() {
            chart.draw_series(crossovers.iter().map(|co| {
                TriangleMarker::new((co.bitrate, co.vmaf), 6, RED.filled())
            }))?;
        }
...

    {
        ...
        for (i, codec) in codecs.iter().enumerate() {
            ...
            chart.draw_series(
                hull.points
                    .iter()
                    .map(|p| Circle::new((p.bitrate, p.vmaf), 4, color.filled())),
            )?;
...

    {
        ...
        for (i, rung) in ladder.rungs.iter().enumerate() {
            ...
            let label = format!(
                "{} {}",
                p.resolution.label(),
                short_codec_name(p.codec.as_str())
            );
...
mod tests {
    ...
    fn rd_curve_produces_png() {
        ...
        let hull = veo_hull::compute_upper(&points);
    ...
    fn per_codec_rd_curve_produces_png() {
        ...
        let per_codec = veo_hull::compute_per_codec(&points);
