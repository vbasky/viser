...
use veo_ffmpeg::{Codec, Resolution};
...
pub fn compute_upper(points: &[Point]) -> Hull {
    ...
    sorted.sort_by(|a, b| {
        a.bitrate.partial_cmp(&b.bitrate).unwrap()
            .then(b.vmaf.partial_cmp(&a.vmaf).unwrap())
    });
...
pub fn compute_per_codec(points: &[Point]) -> std::collections::HashMap<Codec, Hull> {
    let mut by_codec: std::collections::HashMap<Codec, Vec<Point>> = std::collections::HashMap::new();
    for p in points {
        by_codec.entry(p.codec).or_default().push(p.clone());
    }

    by_codec.into_iter().map(|(codec, pts)| {
        (codec, compute_upper(&pts))
    }).collect()
}
