...
use veo_encoding::clean_stale_temp_dirs;

#[derive(Parser)]
#[command(name = "veo", about = "Video Encoding Optimizer")]
struct Cli {
...
async fn main() -> anyhow::Result<()> {
    ...
    // Init logging
    let level: tracing::Level = if cli.verbose {
        tracing::Level::DEBUG
    } else {
        tracing::Level::WARN
    };
...
async fn cmd_encode(args: EncodeArgs) -> anyhow::Result<()> {
    let codec: veo_ffmpeg::Codec = args.codec.parse()?;
    let resolution = if args.width > 0 && args.height > 0 {
        Some(veo_ffmpeg::Resolution::new(args.width, args.height))
    } else {
        None
    };

    let job = veo_ffmpeg::EncodeJob {
        input: args.input,
        output: args.output,
        resolution,
        codec,
        crf: args.crf,
        rate_control: veo_ffmpeg::RateControlMode::Crf,
        target_bitrate: 0.0,
        preset: args.preset,
        extra_args: vec![],
    };

    let (tx, mut rx) = tokio::sync::mpsc::channel::<veo_ffmpeg::Progress>(10);
    ...
    let result = veo_ffmpeg::encode(job, Some(tx)).await?;
    eprintln!();
    println!("\nEncode complete:");
...
async fn cmd_inspect_probe(file: &str) -> anyhow::Result<()> {
    let result = veo_ffmpeg::probe(file).await?;
    ...
    if result.format.bit_rate > 0 {
        println!(
            "Bitrate:  {:.0} kbps",
            result.format.bit_rate as f64 / 1000.0
        );
    }
...
async fn cmd_quality_measure(args: QualityMeasureArgs) -> anyhow::Result<()> {
    let opts = veo_quality::MeasureOpts {
        metrics: vec![
            veo_quality::Metric::Vmaf,
            veo_quality::Metric::Psnr,
            veo_quality::Metric::Ssim,
        ],
    ...
    let result = veo_quality::measure(&args.reference, &args.distorted, opts).await?;
...
async fn cmd_pertitle_analyze(args: PerTitleAnalyzeArgs) -> anyhow::Result<()> {
    ...
    let rate_control = match args.mode.as_str() {
        "qp" => veo_ffmpeg::RateControlMode::Qp,
        _ => veo_ffmpeg::RateControlMode::Crf,
    };

    let cfg = veo_pertitle::Config {
        encoding: veo_encoding::Config {
    ...
    println!("VEO Per-Title Analysis");
    ...
    let (tx, mut rx) = tokio::sync::mpsc::channel::<veo_pertitle::TrialProgress>(10);
    tokio::spawn(async move {
        while let Some(p) = rx.recv().await {
            eprint!(
                "\r  [{}/{}] {} {} CRF {} -> {:.0} kbps, VMAF {:.1}    ",
    ...
    let result = veo_pertitle::analyze(&args.input, cfg, Some(tx)).await?;
    eprintln!();
    ...
    if !result.ladder.rungs.is_empty() {
        println!(
            "\n  Optimized Ladder ({} rungs):",
            result.ladder.rungs.len()
        );
...
async fn cmd_pershot_detect(args: PerShotDetectArgs) -> anyhow::Result<()> {
    let opts = veo_shot::DetectOpts {
        threshold: args.threshold,
        min_duration: Duration::from_secs_f64(args.min_duration),
    };

    println!(
        "Detecting shots: {} (threshold={:.2})",
        args.input, args.threshold
    );
    let shots = veo_shot::detect(&args.input, opts).await?;
...
async fn cmd_pershot_analyze(args: PerShotAnalyzeArgs) -> anyhow::Result<()> {
    ...
    let cfg = veo_pershot::Config {
        ...
        shot_opts: veo_shot::DetectOpts {
            threshold: args.threshold,
            min_duration: Duration::from_secs_f64(args.min_duration),
        },
        ladder_opts: veo_ladder::Opts::default(),
    };

    println!("VEO Per-Shot Analysis\n  Source: {}", args.input);

    let result = veo_pershot::analyze(&args.input, cfg, None).await?;
    println!(
        "  {} shots, {} trials, {:.1}s",
        result.shot_count,
        result.trial_count,
    ...
    if args.target_bitrate > 0.0 && result.shots.len() > 1 {
        let assignments = veo_pershot::trellis_optimize(
            &result.shots,
            &veo_pershot::TrellisOpts {
...
async fn cmd_persegment_analyze(args: PerSegmentAnalyzeArgs) -> anyhow::Result<()> {
    ...
    println!("VEO Segment-Level CRF Adaptation");
    println!("  Source:      {}", args.input);
    println!(
        "  Target VMAF: {:.1} (+/- {:.1})",
        args.target_vmaf, args.tolerance
    );

    let result = veo_persegment::adapt(&args.input, cfg).await?;
    println!(
...
async fn cmd_contextaware_analyze(args: ContextAwareAnalyzeArgs) -> anyhow::Result<()> {
    ...
    println!("VEO Context-Aware Analysis\n  Source:  {}", args.input);
...
async fn cmd_compare(args: CompareArgs) -> anyhow::Result<()> {
    ...
    veo_compare::serve(veo_compare::Opts {
...
fn parse_resolutions(names: &[String]) -> anyhow::Result<Vec<veo_ffmpeg::Resolution>> {
    names
        .iter()
        .map(|n| n.parse::<veo_ffmpeg::Resolution>())
        .collect()
}

fn parse_codecs(names: &[String]) -> anyhow::Result<Vec<veo_ffmpeg::Codec>> {
    names
        .iter()
        .map(|n| n.parse::<veo_ffmpeg::Codec>())
        .collect()
}
