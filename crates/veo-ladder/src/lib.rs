...
fn to_ladder(mut points: Vec<Point>) -> Ladder {
    ...
    let rungs = points.into_iter().enumerate().map(|(i, p)| {
        Rung { point: p, index: i as i32 }
    }).collect();
    Ladder { rungs }
}
...
impl Ladder {
    pub fn bitrate_range(&self) -> (f64, f64) {
        if self.rungs.is_empty() { return (0.0, 0.0); }
    ...
    pub fn savings(&self, fixed_bitrate: f64) -> f64 {
        if self.rungs.is_empty() || fixed_bitrate <= 0.0 { return 0.0; }
        let top = &self.rungs.last().unwrap().point;
        if top.bitrate >= fixed_bitrate { return 0.0; }
