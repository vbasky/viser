...
use veo_ffmpeg::{ffmpeg_path, probe};
...
pub async fn analyze(path: &str, opts: AnalyzeOpts) -> anyhow::Result<Profile> {
    ...
    let select_filter = if subsample > 1 {
        format!("select='not(mod(n\\,{subsample}))',")
    } else {
        String::new()
    };
...
fn parse_complexity_output(output: &str) -> Vec<FrameComplexity> {
    ...
    let mut current = FrameComplexity {
        pts: Duration::ZERO,
        spatial: 0.0,
        temporal: 0.0,
        dct_energy: 0.0,
...
fn aggregate_segments(
    ...
    while seg_start < total_duration {
        ...
        let seg_frames: Vec<&FrameComplexity> = frames.iter()
            .filter(|f| f.pts >= seg_start && f.pts < seg_end)
            .collect();
